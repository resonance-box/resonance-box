...
mod tests {
    ...
    fn new_id() -> Id {
        Id::try_from(Uuid::new_v4().to_string().as_str()).unwrap()
    }
    ...
    fn test_tracks_scenario() {
        ...
        song.add_track(Track::new(new_id(), None));
        song.add_track(Track::new(new_id(), None));
